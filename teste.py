...
async def extracaoDados(contexto, link, semaphore, retries=2):
    ...
    async with semaphore:
        ...
        for attempt in range(retries):
            try:
                response = await pagina.goto(link, timeout=60000)  # 60 segundos de timeout
                ...
                if attempt < retries - 1:
                    ...
                    await asyncio.sleep(2)  # Atraso para evitar sobrecarga
                else:
...
async def processar_links(links, max_concurrent=5):
...
async def salvar_dados(dados_coletados):
    ...
    if salvar_excel == "sim":
        ...
        logging.info(f"Dados salvos em '{ARQUIVO_EXCEL_DADOS}'.")
