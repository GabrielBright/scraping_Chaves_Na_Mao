...
async def extrair_elemento(pagina, xpath, index=0, default="N/A"):
    try:
        ...
        if await elementos.count() > index:
            texto = (await elementos.nth(index).inner_text()).strip()
            logging.debug(f"Extraído '{texto}' de {xpath} (index {index})")
            return texto
        logging.warning(f"Nenhum elemento encontrado para {xpath} (index {index})")
        return default
    except Exception as e:
        logging.error(f"Erro ao extrair {xpath} (index {index}): {str(e)}")
        return default

async def extracaoDados(contexto, link, semaphore, retries=4):
    async with semaphore:
        pagina = await contexto.new_page()
        logging.info(f"Acessando {link}")
        for attempt in range(retries):
            try:
                response = await pagina.goto(link, timeout=120000)
                if response.status != 200:
                    logging.warning(f"Status {response.status} em {link}. Possível bloqueio.")
                    return None
                await pagina.wait_for_load_state('networkidle', timeout=120000)
                await pagina.wait_for_selector('//article//section[2]//div//div[1]//div//span//p//b', timeout=120000)
...
async def salvar_dados(dados_coletados):
    ...
    print(f"Dados a serem salvos: {df.head()}")  # Debug para verificar os dados
